...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner 
    {
        ...
        require(newOwner != address(0), "Owner not real address");
        _owner = newOwner;
        emit OwnerTransferred(_owner);
    }

    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event OwnerTransferred(address _newOwner);
...
contract Pausable is Ownable {
    ...
    {
        require(!_paused, "Contract cannot pause at this time");
        _;
    }
    modifier paused()
    {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), "Owner and transfer address cannot be the same person");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        assert(!(_exists(tokenId) || to == address(0)));
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "From address needs to be the owner of the token");
